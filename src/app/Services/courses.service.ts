...
export class CoursesService {
  ...
  get all_courses_quantity(): number{
    return this._courses.length
  }

  get_course(id: number): ICourse | undefined{
    return this._courses.find( (course: ICourse) => course.id === id )
  }

  search_course(title: string): ICourse[]{
    return this._courses.filter( (course: ICourse) => course.title.toLocaleLowerCase().includes(title) )
  }
}
